...
class BaseMigration extends Migration
{
     ...
     * @return mixed
     */
    function addColumnToBeTranslated($table)
    {
    ...
    public function addingNewPermission($permissionName, $addToAdmin = false)
    {
        $permission = new Permission();
        $permission->name = $permissionName;
        $permission->display_name = ucwords(str_replace("_", " ", $permissionName));
        $permission->description = ucwords(str_replace("_", " ", $permissionName));
        $permission->save();

        if ($addToAdmin) {
            $rolePermission = new PermissionRole();
            $rolePermission->permission_id = $permission->id;
            $rolePermission->role_id = Role::whereName("admin")->first()->id;
            $rolePermission->save();
        }
    }

    public function addingNewRole($roleName)
    {
        $role = new Role();
        $role->name = $roleName;
        $role->display_name = ucwords(str_replace("_", " ", $roleName));
        $role->description = ucwords(str_replace("_", " ", $roleName));
        $role->save();
    }
